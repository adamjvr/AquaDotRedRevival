...
/// Full-version Reaper strategy split recovered from `setupEnemy`:
/// one Reaper uses Hunter strategy ID 0, the other strategy ID 12's
/// random non-reversing cardinal chooser. Demo-only Blocker/Sneaker Reapers
/// are intentionally not generated by the full-version runtime.
enum AquaDotReaperBehavior: String, Codable, Equatable, Sendable {
    case hunter
    case random
}
...
struct AquaDotBugState: Equatable, Sendable {
    ...
    /// actual strategy; this flag changes only the drawn bug sprite. Reapers are
    /// never Neon in the shipped `setupEnemy` path.
    var isNeonAppearance: Bool

    /// Phase 4 authenticity closure: non-nil marks the special Night/Reaper path.
    /// Hunter Reapers retain `.hunter` as the ordinary strategy adapter; Random
    /// Reapers dispatch through the recovered strategy-12 chooser.
    var reaperBehavior: AquaDotReaperBehavior?

    /// Per-segment velocity factor. Normal corridors use 1.0; the first corridor
    /// after a warp uses the exact recovered `warpStyle` multiplier.
    var segmentSpeedMultiplier: Double
    ...
    init(
        ...
        segmentSpeedMultiplier: Double = 1.0,
        ...
        self.segmentSpeedMultiplier = segmentSpeedMultiplier
    ...
    var isReaper: Bool { reaperBehavior != nil }
